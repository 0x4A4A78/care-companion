import { CalendarDays, ChevronLeft, Clock3, MapPin, ShieldCheck, UserRound } from "lucide-react";
...
export default async function CompanionJobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="page-wrap">
    ...
    <div className="grid-main"><div className="stack">
      <Card className="form-card"><h2>รายละเอียดการเดินทาง</h2><div className="summary-list"><div className="summary-row"><span><CalendarDays size={17} /> นัดหมาย</span><strong>{formatThaiDate(request.serviceDate)} เวลา {request.startTime} น.</strong></div><div className="summary-row"><span><Clock3 size={17} /> ระยะเวลา</span><strong>ประมาณ {request.durationHours} ชั่วโมง</strong></div><div className="summary-row"><span><MapPin size={17} /> ต้นทาง</span><strong>{request.pickup}</strong></div><div className="summary-row"><span><MapPin size={17} /> จุดหมาย</span><strong>{request.destination}</strong></div><div className="summary-row"><span>ต้องการให้ช่วย</span><strong>{request.supportNeeds.length ? request.supportNeeds.join(", ") : "ช่วยเหลือทั่วไป"}</strong></div>{request.notes && <div className="summary-row"><span>หมายเหตุ</span><strong>{request.notes}</strong></div>}</div></Card>
      {isMine && <JobChatSection requestId={request.id} companionName={request.customerName} initialMessages={messages} title="ข้อความกับ Customer" />}
